...
Deno.serve(async (req) => {
  ...
  const { error } = await admin.from("verification_documents").insert({
    provider_id: prov?.id ?? null, user_id: user.id, doc_type: "insurance", status: "pending",
    file_paths: b.insurancePaths ?? [], policy_expiry: b.policyExpiry ?? null,
    coverage_amount_cents: b.coverageAmountCents ?? null,
  });
