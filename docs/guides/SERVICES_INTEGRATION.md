...
1. **No pay-per-lead, ever.** Each new vertical inherits the local provider marketplace model — providers never pay to receive or respond to a request. The browse "neighborhood pros" filter (cross-cutting §5) is for *discovery*; homeowner sees local, verified providers with trust scores and quotes, then chooses. Nobody pays until the job is verified complete.
