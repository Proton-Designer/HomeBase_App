...
export default function ProviderThreadScreen() {
  ...
  // Default to provider_owner if role isn't set yet (mock sessions).
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToMessages(jobId, () => {
      ...
      void queryClient.invalidateQueries({ queryKey: ['threads', 'provider'] });
    });
    return unsub;
  }, [jobId, queryClient]);
  ...
  const sendM = useMutation({
    mutationFn: (body: string) => sendMessage({ jobId: jobId!, body, fromRole }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['messages', jobId] });
      void queryClient.invalidateQueries({ queryKey: ['threads', 'provider'] });
    },
  ...
  const headerName = useMemo(() => {
    ...
    return other?.fromRole === 'homeowner' ? 'Homeowner' : 'Thread';
  }, [messages, userId]);
