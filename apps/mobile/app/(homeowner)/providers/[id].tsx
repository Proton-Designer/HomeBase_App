...
export default function ProviderDetailScreen() {
  ...
  const handleBook = () => {
    ...
    // Pre-select this pro so the match step can offer one-tap rehire.
    setMatchedProvider(provider.id);
                ...
                {provider.externalRating ? (
                  ...
                  >
                    <Text style={{ ...textStyles['body-sm'], color: colors.textSecondary }}>
                      ★ Established
                      {provider.externalYears ? ` · ${provider.externalYears} yrs` : ''} ·{' '}
                      {provider.externalRating.toFixed(1)} on {provider.externalSource ?? 'Google'}
                    </Text>
                  </View>
                ) : null}
              </Card>
