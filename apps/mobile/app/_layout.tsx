...
function AuthBootstrap({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const registerToken = async () => {
      try {
        ...
        // expo-notifications exposes its functions as namespace exports (NOT on
        // `.default`, which is undefined at runtime).
  ...
  // Tapping a push notification deep-links into the notification center for the
  // current role (so the alert isn't fire-and-forget).
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // expo-notifications can throw on simulators / web — safe to ignore.
      }
    })();
    return () => {
      active = false;
