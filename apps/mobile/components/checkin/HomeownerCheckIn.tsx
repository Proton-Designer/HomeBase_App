...
export function HomeownerCheckIn({
  ...
  const submit = async () => {
    if (state.photoUploading || submitting) return;
    const { photoUri: _photoUri, photoUploading: _photoUploading, ...submitState } = state;
    setSubmitting(true);
    // Persist the review BEFORE celebrating — otherwise a failed submit silently
    // loses the review behind a success screen.
    try {
      await Promise.resolve(onSubmit?.(submitState));
    } catch (e) {
      console.error('[HomeownerCheckIn] onSubmit failed:', e);
      setSubmitting(false);
      Alert.alert('Review not saved', 'We couldn’t save your review. Please try again.');
      return;
    }
    setSubmitting(false);
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
    setStep(6);
    // Payment capture is best-effort — the review is already saved.
    captureOnCompletion({ jobId }).catch((e) => {
  ...
  const stepContent = (
    <View testID="checkin-step-container" style={{ flex: 1, padding: 20, justifyContent: 'center' }}>
      {step === 0 ? (
          ...
          {step === 5 ? (
            <Animated.View key="photo" entering={onlyNative(SlideInRight.duration(260))} exiting={onlyNative(SlideOutLeft)}>
              <Text
                style={{
              ...
              {state.photoUri ? (
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  ...
                  />
                  <Pressable
                    testID="checkin-ho-photo-remove"
                ...
                <Pressable
                  testID="checkin-ho-photo-capture"
                  onPress={onPickPhoto}
                  style={{
                    height: 140,
                    borderRadius: 14,
              ...
              <View style={{ marginTop: 24, gap: 10 }}>
                <Button
                  ...
                  onPress={submit}
                />
                <Pressable testID="checkin-ho-photo-skip" onPress={submit} hitSlop={6}>
                  <Text
                    style={{
                      ...textStyles['body-md'],
                      fontFamily: 'Inter_500Medium',
  ...
  return (
    <Animated.View entering={onlyNative(SlideInRight.duration(260))} exiting={onlyNative(SlideOutLeft)}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginVertical: 12 }}>
        {[1, 2, 3, 4, 5].map((n) => (
          <Pressable key={n} testID={`checkin-ho-rating-star-${n}`} onPress={() => onChange(n)} hitSlop={10}>
            <Star
              size={48}
      ...
      <View style={{ marginTop: 36 }}>
        <Button testID="checkin-step-next" label="Continue" size="lg" fullWidth disabled={value === 0} onPress={onContinue} />
      </View>
    </Animated.View>
  );
}
  ...
  return (
    <Animated.View entering={onlyNative(FadeIn.duration(280))} style={{ alignItems: 'center', gap: 16 }}>
      ...
      <View style={{ marginTop: 32, alignSelf: 'stretch' }}>
        <Button testID="checkin-ho-done" label="Done" size="lg" fullWidth onPress={onDone} />
      </View>
    </Animated.View>
  );
}
